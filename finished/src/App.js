...
import GearRenderer from "./components/GearRenderer";
...
function App() {
  ...
  const levelUpGear = (_account, _id) => {
    setLoading(true);
    blockchain.GearToken.methods
      .levelUp(_id)
      .send({
  ...
  useEffect(() => {
    if (blockchain.account != "" && blockchain.GearToken != null) {
      dispatch(fetchData(blockchain.account));
    }
  }, [blockchain.GearToken]);
      ...
      {blockchain.account === "" || blockchain.GearToken === null ? (
        ...
        <s.Container ai={"center"} style={{ padding: "24px" }}>
          ...
          <s.Container jc={"center"} fd={"row"} style={{ flexWrap: "wrap" }}>
            {data.allGears.map((item, index) => {
              return (
                <s.Container key={index} style={{ padding: "15px" }}>
                  <GearRenderer Gear={item} />
                  <s.SpacerXSmall />
                  <s.Container>
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        levelUpGear(blockchain.account, item.id);
                      }}
                    >
                      Level Up
                    </button>
